...
from vvhgvs.exceptions import HGVSError, HGVSUsageError, HGVSDataNotAvailableError
from vvhgvs.utils import build_tx_cigar
...
class TranscriptMapper(object):
    ...
    recommendations.  All methods take :class:`vvhgvs.location.Interval`
    objects.

    :param hdp: HGVS Data Provider Interface-compliant instance (see :class:`vvhgvs.dataproviders.interface.Interface`)
    ...
    def __init__(self, hdp, tx_ac, alt_ac, alt_aln_method):
        ...
        if self.alt_aln_method != "transcript":
            ...
            self.tx_identity_info = hdp.get_tx_limits(self.tx_ac)
            ...
            self.tgt_len = self.tx_identity_info["length"]
    ...
    def __str__(self):
        return "{self.__class__.__name__}: {self.tx_ac} ~ {self.alt_ac} ~ {self.alt_aln_method); " \
               "{strand_pm} strand; {n_exons} exons; offset={self.gc_offset}".format(
                   self=self, n_exons=len(self.cigar), strand_pm=strand_int_to_pm(self.strand))
    ...
    def g_to_n(self, g_interval):
        ...
        # confuses interbase intervals for a *single* vvhgvs position
        # with intervals of *two* vvhgvs positions.  This is the origin
        ...
        return vvhgvs.location.BaseOffsetInterval(
            start=vvhgvs.location.BaseOffsetPosition(base=start_bo[0], offset=start_bo[1], datum=Datum.SEQ_START),
            end=vvhgvs.location.BaseOffsetPosition(base=end_bo[0], offset=end_bo[1], datum=Datum.SEQ_START),
    ...
    def n_to_g(self, n_interval):
        ...
        return vvhgvs.location.Interval(
            start=vvhgvs.location.SimplePosition(_ci_to_hgvs_coord(gs, ge)[0], uncertain=n_interval.start.uncertain),
            end=vvhgvs.location.SimplePosition(_ci_to_hgvs_coord(gs, ge)[1], uncertain=n_interval.end.uncertain),
    ...
    def n_to_c(self, n_interval):
        ...
        c_interval = vvhgvs.location.BaseOffsetInterval(
            start=vvhgvs.location.BaseOffsetPosition(base=cs, offset=n_interval.start.offset, datum=cs_datum),
            end=vvhgvs.location.BaseOffsetPosition(base=ce, offset=n_interval.end.offset, datum=ce_datum),
            uncertain=n_interval.uncertain)
        return c_interval
    ...
    def c_to_n(self, c_interval):
        ...
        n_interval = vvhgvs.location.BaseOffsetInterval(
            start=vvhgvs.location.BaseOffsetPosition(base=rs, offset=c_interval.start.offset, datum=Datum.SEQ_START),
            end=vvhgvs.location.BaseOffsetPosition(base=re, offset=c_interval.end.offset, datum=Datum.SEQ_START),
