...
from vvhgvs.utils.norm import normalize_alleles
from vvhgvs.exceptions import HGVSDataNotAvailableError, HGVSUnsupportedOperationError, HGVSInvalidVariantError
...
class Normalizer(object):
    ...
    def __init__(self,
                 hdp,
                 cross_boundaries=vvhgvs.global_config.normalizer.cross_boundaries,
                 shuffle_direction=vvhgvs.global_config.normalizer.shuffle_direction,
                 alt_aln_method=vvhgvs.global_config.mapping.alt_aln_method,
                 validate=vvhgvs.global_config.normalizer.validate):
        """Initialize and configure the normalizer

        :param hdp: HGVS Data Provider Interface-compliant instance
            (see :class:`vvhgvs.dataproviders.interface.Interface`)
        ...
        self.validator = None
        if validate:
            self.validator = vvhgvs.validator.IntrinsicValidator(strict=False)
    ...
    def normalize(self, var):
        ...
        assert isinstance(var,
                          vvhgvs.sequencevariant.SequenceVariant), "variant must be a parsed HGVS sequence variant object"
        ...
        if alt_len < ref_len:
            ...
            ref_start = start
            ref_end = end - 1
            edit = vvhgvs.edit.NARefAlt(ref=ref, alt=None if alt_len == 0 else alt)
    ...
    def _fetch_bounded_seq(self, var, start, end, window_size, boundary):
        """Fetch reference sequence from vvhgvs data provider.
    ...
    def _normalize_alleles(self, var, boundary):
        ...
        win_size = vvhgvs.global_config.normalizer.window_size
